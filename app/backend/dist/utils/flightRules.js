...
exports.parseFlightRules = parseFlightRules;
function parseFractionSm(s) {
    if (s.includes('/')) {
...
function parseFlightRules(metar) {
    ...
    if (visMixed)
        visSm = parseInt(visMixed[1]) + parseInt(visMixed[2]) / parseInt(visMixed[3]);
    else if (visFrac)
        visSm = parseFractionSm(visFrac[1]);
    else if (visFull)
        visSm = parseFloat(visFull[1]);
    ...
    if (ceilingFt < 500 || visSm < 1)
        return 'LIFR';
    if (ceilingFt < 1000 || visSm < 3)
        return 'IFR';
    if (ceilingFt < 3000 || visSm < 5)
        return 'MVFR';
    return 'VFR';
}
