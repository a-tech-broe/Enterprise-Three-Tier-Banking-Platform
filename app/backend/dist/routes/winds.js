...
function parseGroup(raw, altFt) {
    const t = raw.trim();
    const level = { altFt, dir: null, speed: null, tempC: null };
    if (!t)
        return level;
    if (t === '9900') {
        level.dir = 0;
...
function icaoToStation(icao) {
    const u = icao.toUpperCase();
    return /^[KPA][A-Z]{3}$/.test(u) ? u.slice(1) : u;
}
function parseWindsText(text, station) {
...
router.get('/:icao', async (req, res, next) => {
    try {
        ...
        if (!response.ok)
            throw new Error(`windtemp fetch error: ${response.status}`);
        const text = await response.text();
        const result = parseWindsText(text, station);
        if (!result)
            return res.json({ station: null, icao, validTime: null, levels: [] });
        result.icao = icao;
        res.json(result);
    }
    catch (err) {
