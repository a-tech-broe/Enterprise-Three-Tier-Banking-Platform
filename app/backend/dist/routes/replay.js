...
router.get('/tracked', async (_req, res, next) => {
    try {
        const { rows } = await pool_1.pool.query('SELECT icao FROM tracked_airports ORDER BY icao');
...
router.post('/track', async (req, res, next) => {
    try {
        ...
        await pool_1.pool.query('INSERT INTO tracked_airports (icao) VALUES ($1) ON CONFLICT DO NOTHING', [icao.toUpperCase()]);
        res.json({ ok: true });
    }
    catch (err) {
        next(err);
...
router.delete('/track/:icao', async (req, res, next) => {
    try {
        await pool_1.pool.query('DELETE FROM tracked_airports WHERE icao = $1', [
...
router.get('/:icao', async (req, res, next) => {
    try {
        ...
        const { rows } = await pool_1.pool.query(`SELECT captured_at, flight_rules, raw_metar, wdir, wspd, wgst, visib, temp, altim
       FROM obs_snapshots
       WHERE icao = $1 AND captured_at > NOW() - ($2 * INTERVAL '1 hour')
       ORDER BY captured_at ASC`, [icao, hours]);
        res.json(rows);
    }
    catch (err) {
        next(err);
