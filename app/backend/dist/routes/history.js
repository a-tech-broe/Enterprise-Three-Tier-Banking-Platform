...
router.get('/', async (req, res, next) => {
    try {
        ...
        if (!deviceId || !type || !VALID_TYPES.has(type))
            return res.json([]);
        // Deduplicate by ICAO — keep most recent hit per airport, return top 10
        const result = await pool_1.pool.query(`SELECT DISTINCT ON (icao) icao, searched_at
       FROM search_history
       WHERE device_id = $1 AND search_type = $2
       ORDER BY icao, searched_at DESC`, [deviceId, type]);
...
router.post('/', async (req, res, next) => {
    try {
        const { deviceId, icao, type } = req.body;
        if (!deviceId || !icao || !type || !VALID_TYPES.has(type)) {
            return res.status(400).json({ error: 'Missing or invalid fields' });
        }
        await pool_1.pool.query(`INSERT INTO search_history (device_id, icao, search_type) VALUES ($1, $2, $3)`, [deviceId, icao.toUpperCase().slice(0, 6), type]);
